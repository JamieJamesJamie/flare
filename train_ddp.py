...
def main():
    ...
    print(
        f"IN train_ddp.main()--RANK: {rank};  MY DEVICE: {device};  WORLD_SIZE: {world_size}"
    )
    if rank == 0:
    ...
    if "pixel" in args.encoder_type:
        ...
        obs_shape = (
            number_channel * args.frame_stack,
            args.image_size,
            args.image_size,
        )
        pre_aug_obs_shape = (
    ...
    for step in range(0, args.num_train_steps, world_size):  # increment by world_size
        ...
        if step >= args.init_steps:
            ...
            for step_incr in range(num_updates):
                agent.update(
                    replay_buffer, L, step + step_incr
                )  # step arg increments by one.
